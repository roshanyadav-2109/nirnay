...
export async function generateReportPdf(opts: BuildOpts): Promise<BuildResult> {
  ...
  // Footer on every page (no watermark — keeps the report visually clean)
...
function buildCoverPage(d: jsPDF, margin: number, data: CoverData) {
  ...
  d.setFont('helvetica', 'normal');
  d.setFontSize(10);
  d.text('Prepared by the Procurement Evaluation Cell using the Nirnay system', w / 2, margin + 164, { align: 'center' });
...
async function buildSignaturePage(d: jsPDF, margin: number, data: SignatureData) {
  ...
  d.setFont('helvetica', 'bold');
  d.setFontSize(11);
  d.text('4. AUTHENTICATION', margin, y);
  y += 6;
  ...
  y = paragraph(
    ...
    margin,
    y,
    'This report is authenticated through a tamper-evident audit trail rather than a traditional pen-and-ink or token-based digital signature. Every action that produced the verdicts in this report — from uploading the tender, to extracting eligibility criteria, to finalising each verdict and any officer override — has been recorded in order, with a unique reference code that depends on every event before it.',
    { lineHeight: 13 },
  );
  y += 4;
  y = paragraph(
    ...
    margin,
    y,
    'If any part of this report, or any earlier action in the audit trail, is modified after this report has been issued, those reference codes will no longer match and the change will be visible at a glance. The codes printed below let any officer or auditor verify that this report has not been altered since the date and time of its generation.',
    { lineHeight: 13 },
  );
  y += 8;
  ...
  d.setFont('helvetica', 'bold');
  d.setFontSize(10);
  d.text('VERIFICATION CODES', margin + 16, y);
  y += 16;
  d.setFont('helvetica', 'normal');
  d.setFontSize(8.5);

  d.setTextColor(80, 80, 80);
  d.text('Audit-trail anchor (latest event reference):', margin + 16, y);
  y += 11;
  d.setTextColor(0, 0, 0);
  d.setFont('courier', 'normal');
  d.setFontSize(8);
  ...
  d.setFontSize(8.5);
  d.setTextColor(80, 80, 80);
  d.text('Document fingerprint:', margin + 16, y);
  y += 11;
  d.setTextColor(80, 80, 80);
  d.setFont('helvetica', 'italic');
  d.setFontSize(8);
  d.text('Recomputed from this PDF on demand. The fingerprint must match the', margin + 16, y);
  y += 10;
  d.text('one recorded against the file number above in the audit trail.', margin + 16, y);
  y += 14;

  d.setFont('helvetica', 'normal');
  d.setFontSize(8.5);
  d.setTextColor(80, 80, 80);
  d.text('How to verify this report:', margin + 16, y);
  y += 11;
  d.setTextColor(0, 0, 0);
  d.setFontSize(8);
  d.text('1.  Open the Audit Trail in the Nirnay dashboard.', margin + 16, y);
  y += 10;
  d.text('2.  Filter the events by the File Number printed at the top of this report.', margin + 16, y);
  y += 10;
  d.text('3.  Locate the report-generation event and compare its document fingerprint', margin + 16, y);
  y += 10;
  d.text('     with the fingerprint computed from this PDF.', margin + 16, y);
  y += 10;
  d.text('4.  Click "Verify chain" — the dashboard re-checks every event in order.', margin + 16, y);
  ...
  d.text('Authenticated by the Nirnay audit trail', margin, y + 46);
